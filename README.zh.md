...
bot 一接收到消息，就会在这条消息上加一个表情，默认是 📌 图钉。起任务的那条有，更要紧的是另外两种：排在正在跑的任务后面的消息，以及追问进正在跑的任务的消息。它们在 agent 处理到之前什么动静都没有，追问一分钟没反应，和根本没送到看起来一模一样，这个表情就是用来区分这两种情况的。

反过来也算数：如果 bot 最终没有处理这条消息（被 `/stop` 或其他清队列的命令丢掉了，或者它等的那次运行没能启动），表情会撤回。表情还在，就说明消息已经到了 agent 手里，或者还会到。命令不打表情，命令的回复本身就是回执。

在 `/config` 的 **收到回执** 里换贴纸或关掉；也可以直接在 `config.json` 的 `preferences.ackReaction` 里写任意飞书 reaction 的 `emoji_type`（`"Pin"`、`"Get"`、`"OK"`、`"THUMBSUP"` …）或 `false`。回执在消息入队时就打上，与背后的 agent 无关，Claude 和 Codex profile 一视同仁。
...
**agent 子进程假死（卡片停在最后一帧不动）**：有两层互补的探活。

- **工具卡死探活（`toolStallTimeoutMinutes`，默认开启）**：覆盖 run 卡在某个工具调用上不返回的情况——阻塞的 Bash、等输入的交互命令、卡住的 MCP。两段式：默认静默 20 分钟后先在卡片上标出「工具 X 已 20 分钟无输出」并保留 ⏹ 终止按钮，**不杀进程**；再过 10 分钟（`toolStallGraceMinutes`）仍无任何事件才 SIGTERM，卡片标注终止原因。合法的长任务（OAuth 授权、长时间编译）因此有完整 30 分钟窗口，且在被杀之前你已经看得到。任一事件（包括慢工具最终返回）都会清掉告警并重置计时。设为 `0` 完全关闭。
- **idle 探活（`runIdleTimeoutMinutes`，默认关闭）**：agent 一段时间没输出就 SIGTERM。它在有工具调用未返回时**会主动暂停**，避免误杀长任务——这正是上面那层存在的原因。开启方式：`/config` 设全局值（分钟），或 `/timeout 10` 只对当前会话生效；`/timeout off` 关掉当前会话的探活；`/timeout default` 清掉会话覆盖，回退到全局设置。

**回复没送达（卡片停在流式状态，但 agent 其实已经跑完）**：卡片的流式更新失败时（超大卡片 400、限流、卡片序列号冲突、网络抖动），bridge 会把本轮已积累的完整回复作为一条普通消息补发，开头带「⚠️ 消息流式更新失败」。收到这条就说明本轮**已经结束**，正文即完整结果——它可能和卡片上已显示的内容部分重复。
