...
bot 一接收到消息，就会在这条消息上加一个表情，默认是飞书的「GET / 收到」贴纸。起任务的那条有，更要紧的是另外两种：排在正在跑的任务后面的消息，以及追问进正在跑的任务的消息。它们在 agent 处理到之前什么动静都没有，追问一分钟没反应，和根本没送到看起来一模一样，这个表情就是用来区分这两种情况的。

反过来也算数：如果 bot 最终没有处理这条消息（被 `/stop` 或其他清队列的命令丢掉了，或者它等的那次运行没能启动），表情会撤回。表情还在，就说明消息已经到了 agent 手里，或者还会到。命令不打表情，命令的回复本身就是回执。

在 `/config` 的 **收到回执** 里换贴纸或关掉；也可以直接在 `config.json` 的 `preferences.ackReaction` 里写任意飞书 reaction 的 `emoji_type`（`"Get"`、`"OK"`、`"THUMBSUP"` …）或 `false`。
