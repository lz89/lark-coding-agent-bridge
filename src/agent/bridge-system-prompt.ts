...
需要跨多轮才能干完的活，正确做法是**闭环模式**：让用户发 \`/goal <目标>\`，bridge 会在每轮结束后自动再给你一轮，同一个 session、上下文全在。那时你会在本轮 prompt 里收到「闭环模式」小节和当轮的信号文件路径，按那里的说明走。没收到那个小节，就说明当前**不在**闭环模式里，本轮结束就是真的结束。
